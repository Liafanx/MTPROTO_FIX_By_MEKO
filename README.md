...
1. Установить стандартный Telemt версии 3.4.18 (версия новее работает нестабильно, появляются реконнекты и медиа могут грузить хуже.)
Либо альтернативой вы можете поставить "MTPROTO.zig", но тогда при установке необходимо указать --no-dpi, например sudo mtbuddy install --port 443 --domain rutube.ru --no-tcpmss --yes
